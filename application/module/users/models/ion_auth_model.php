...
class Ion_auth_model extends CI_Model {
	...
	public function hash_password($password, $salt = false, $use_sha1_override = FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'ci_encrypt')
		{
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'ci_encrypt')
		{
			if ($password == $this->encrypt->decode($hash_password_db->password))
			{
				return TRUE;
			}
	...
	public function forgot_password($identity)
	{
		...
		$query = $this->db->get_where($this->tables['users'],array('email' => $identity));
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login,user_type')
	...
	public function get_users_groups($id = FALSE)
	{
		...
		$query= $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')
						->where($this->tables['users_groups'] . '.' . $this->join['user'], $id)
	 ...
	 *
	 * @return void
	 * @author Bhumika Sachula
	 * */
	public function set_password($identity, $old, $new){
		$data = array('password' => $new);					
		$this->db->where('user_id', $identity);			
